...
if not SKIP_CUDA_BUILD:
    ...
    if HIP_BUILD:
        ...
        extra_compile_args = {
            ...
            "nvcc": [
                "-O3",
                "-std=c++17","-allow-unsupported-compiler","-allow-unsupported-compiler","-allow-unsupported-compiler",
    ...
    extra_includes = [Path(this_dir) / "csrc" / "selective_scan"]
    # Add CUDA target headers if available (needed for <nv/target>)
    cuda_home = os.environ.get("CUDA_HOME") or CUDA_HOME
    if cuda_home:
        cuda_target_include = Path(str(cuda_home)) / "targets" / "x86_64-linux" / "include"
        if cuda_target_include.is_dir():
            extra_includes.append(cuda_target_include)
