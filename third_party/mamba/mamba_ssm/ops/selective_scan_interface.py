...
try:
    ...
    # Preferred import path used by some versions of causal-conv1d
    ...
    # Fallback for wheels/layouts that expose only the low-level CUDA API
    try:
        import causal_conv1d_cuda  # type: ignore

        def causal_conv1d_fwd_function(x, w, b, seq_idx, init_states, final_states_out, activation):
            return causal_conv1d_cuda.causal_conv1d_fwd(x, w, b, seq_idx, init_states, final_states_out, activation)

        def causal_conv1d_bwd_function(x, w, b, dout, seq_idx, init_states, dfinal_states, dx_out, return_dinit, activation):
            return causal_conv1d_cuda.causal_conv1d_bwd(
                x, w, b, dout, seq_idx, init_states, dfinal_states, dx_out, return_dinit, activation
            )

        def causal_conv1d_update_function(x, state, w, b, activation):
            return causal_conv1d_cuda.causal_conv1d_update(x, state, w, b, activation)
    except Exception:
...
# Try to import CUDA kernel; fall back to ref ops when unavailable
try:
...
def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    """
    if not _HAS_SELECTIVE_SCAN_CUDA:
...
def mamba_inner_fn(
    ...
    if not _HAS_SELECTIVE_SCAN_CUDA:
        return mamba_inner_ref(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                               out_proj_weight, out_proj_bias, A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
