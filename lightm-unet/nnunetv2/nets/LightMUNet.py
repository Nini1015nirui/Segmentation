...
# On CPU-only environments, ensure mamba uses pure PyTorch ops (disable fused CUDA paths)
try:
    if not torch.cuda.is_available():
        import mamba_ssm.modules.mamba_simple as _mamba_simple  # type: ignore
        # Force CPU-safe path: use nn.Conv1d instead of causal_conv1d CUDA kernels
        _mamba_simple.causal_conv1d_fn = None
        _mamba_simple.causal_conv1d_update = None
        # Also force selective scan to use pure PyTorch reference instead of CUDA kernel
        import mamba_ssm.ops.selective_scan_interface as _ssi  # type: ignore
        _ssi._HAS_SELECTIVE_SCAN_CUDA = False
        _ssi.selective_scan_cuda = None
        # Ensure mamba module uses reference implementation on CPU
        try:
            _mamba_simple.selective_scan_fn = _ssi.selective_scan_ref
        except Exception:
            pass
except Exception:
    pass
...
class MambaLayer(nn.Module):
    def __init__(self, input_dim, output_dim, d_state = 16, d_conv = 4, expand = 2):
        ...
        self.mamba = Mamba(
                ...
                use_fast_path=torch.cuda.is_available(),
        )
